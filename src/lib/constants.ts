...
export const CONDITIONS = [
  ...
  new Condition(
    ...
    (w: string) => /^[a-z]/i.test(w) && !/^[aeiou]/i.test(w)
  ),
  ...
  new Condition(
    'consonant_heavy',
    'Fewer than 2 vowels',
    (w: string) => (w.match(/[aeiou]/gi) || []).length < 2
  ),
  ...
  new Condition(
    'many_vowels',
    '3+ vowels',
    (w: string) => (w.match(/[aeiou]/gi) || []).length >= 3
  ),
  new Condition(
    'many_unique',
    '4+ unique letters',
    (w: string) => new Set(w.replace(/[^a-z]/gi, '').split('')).size >= 4
  ),
